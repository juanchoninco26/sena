...
<html lang="es">
  <head>
    ...
    <title>Correos suscritos al newsletter de ARED</title>
  </head>
  <body>
...
<div class="container mt-5"> 
  <h3 class="text-center mb-5" style="font-weight: 800; font-size: 35px">
    Correos suscritos al newsletter de ARED
    <hr>
  </h3>
...
include('puntos-turista-bd.php');
  $QueryInmuebleDetalle = ("SELECT * FROM correo ");
  $resultadoInmuebleDetalle = mysqli_query($ared, $QueryInmuebleDetalle);
        ...
        <tbody>
          ...
          while ($dataClientes = mysqli_fetch_array($resultadoInmuebleDetalle)) { ?>
            <tr>
                ...
                <td>
                  <?php
                  echo ($dataClientes['Activo']) ? '<i class="zmdi zmdi-check-all zmdi-hc-2x green"></i>' : '<i class="zmdi zmdi-check zmdi-hc-2x black"></i>';
                  ?>
                </td>
            </tr>
          <?php $i++; } ?>
